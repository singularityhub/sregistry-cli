...
from .query import search, search_all, container_search
...
class Client(ApiConnection):
        ...
        try:
            ...
            if error_code == 403:
                if not self._id or not self._key:
                    bot.exit(
                        "The {} bucket is not public and needs AWS credentials to be accessed".format(
                            self.bucket_name
                        )
                    )
                bot.exit(
                    "The {} bucket cannot be accessed with the provided AWS credentials".format(
                        self.bucket_name
                    )
                )
            elif error_code == 404:
                ...
                if confirm_action(
                    "Are you sure you want to create the {} bucket?".format(
                        self.bucket_name
                    )
                ):
                    if not self._id or not self._key:
                    ...
                    try:
                        ...
                        bot.exit(
                            "Could not create bucket {}: {}".format(
                                self.bucket_name, str(e)
                            )
                        )
                else:
    ...
    def _update_secrets(self, base=None):
        ...
        if not self._id or not self._key:
            bot.warning(
                "Accessing the bucket anonymously. Consider defining AWS_ACCESS_KEY_ID and AWS_SECRET_ACCESS_KEY if access fails."
            )
