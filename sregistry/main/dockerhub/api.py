...
def get_manifests(self, repo_name, digest=None):
    ...
    arch = "SREGISTRY_DOCKERHUB_ARCHITECTURE"
    DOCKER_OS = self._get_and_update_setting("SREGISTRY_DOCKERHUB_OS", "linux")
...
def download_layers(self, repo_name, digest=None, destination=None):
    ''' download layers is a wrapper to do the following for a client loaded
        ...
        at the same time.
    '''
    from sregistry.main.base import Workers
    from sregistry.main.dockerhub.tasks import download_layer_task
    ...
    for digest in digests:
        ...
        if not os.path.exists(targz):
            tasks.append((worker, digest, repo_name, destination))
        layers.append(targz)

    # Download layers with multiprocess workers
    if len(tasks) > 0:
        download_layers = workers.run(func=download_layer_task,
                                      tasks=tasks)
...
def extract_runscript(self):
    ...
    '''
    use_cmd = self._get_setting('SREGISTRY_DOCKERHUB_CMD')
