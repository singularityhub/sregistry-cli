...
def authorize(self, names, payload=None, request_type="push"):
    ...
    if self.username is None:
        ...
        if self.username is None:
            bot.exit(
                "Failed to authorize: please set SREGISTRY_REGISTRY_USERNAME to an appropriate value"
            )
        else:
    ...
    if self.token is None:
        ...
        if self.token is None:
            bot.exit(
                "Failed to authorize: please set SREGISTRY_REGISTRY_TOKEN to an appropriate value"
            )
        else:
