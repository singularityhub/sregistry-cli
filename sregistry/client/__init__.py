...
def get_parser():
    ...
    backend = subparsers.add_parser(
        "backend", help="list, remove, or activate a backend."
    )
    ...
    images.add_argument(
        "query", nargs="*", help="image search query", type=str, default="*"
    )
    ...
    rename = subparsers.add_parser("rename", help="rename an image in storage")

    rename.add_argument(
        "name", help="image name or uri to rename in database", type=str
    )

    rename.add_argument(
        "path", help="path to rename image (will use basename)", type=str
    )
    ...
    # Build an image
    build = subparsers.add_parser("build", help="build an image using a remote.")
    ...
    # Push an image
    push = subparsers.add_parser("push", help="push one or more images to a registry")

    push.add_argument("image", help="full path to image file", type=str)
    ...
    # Share an image
    share = subparsers.add_parser("share", help="share a remote image")

    share.add_argument("image", help="full uri of image", type=str)
    ...
    # Pull an image
    pull = subparsers.add_parser("pull", help="pull an image from a registry")

    pull.add_argument("image", help="full uri of image", type=str)

    pull.add_argument(
        "--name", dest="name", help="custom name for image", type=str, default=None
    )
